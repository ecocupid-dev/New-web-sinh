...
import { categorySortOptions, countryList } from "@/config";
...
export const CategoryFilterContent = ({ handleFilter }: TProps) => {
  const { control, reset, handleSubmit, setValue, watch } = useForm<
    TCategoryFilter & { TypeSort?: number }
  >({
    mode: "onBlur",
  });

  const handlePress = async (data: TCategoryFilter) => {
  ...
  return (
    <div className="w-[500px] grid grid-cols-2 gap-4">
      ...
      <SelectForm
        ...
        name="Sort"
        label="Sort"
        dataOptions={categorySortOptions}
      />
